...
•	app.py, Dockerfile, requirements.txt, and .gitlab-ci.yml
Key decision: Used the existing structure without modification to ensure compatibility.
...
make test
make run
Key decision: Confirmed the code works locally to isolate issues before GitLab pipeline testing.
________________________________________
...
•	flake8==3.9.0
•	pytest==6.2.2
Key decision: I used the requirements.txt file instead of manually listing packages from the code, since it’s the official and standard way to define Python dependencies. This also aligns with how they’re installed in Docker and CI/CD environments.
________________________________________
...
Code:
cat Dockerfile
Key decision: Kept port as-is for consistency with Docker exposure later.
________________________________________
...
Key decision: Tested locally before containerizing to ensure functionality.
________________________________________
...
Key decision: Used make test to run tests and for consistency.
________________________________________
...
Key decision: Used Docker-in-Docker (docker:dind) service for building and pushing inside CI; ensured secrets were never exposed in logs.
________________________________________
...
Key decision: Used GitLab’s web UI to closely monitor each pipeline stage. Broke pipeline into multiple stages (test, build) for easier troubleshooting. Repeated pipeline runs to confirm fixes and successful execution.
________________________________________
...
Key decision: Adding a deploy stage completes the pipeline, automating the full path from code commit to live app.
...
•	Generated an SSH key pair locally if not already available:
...
Error encountered: SSH access failed due to file permissions → fixed with:
...
•	Used the private SSH key stored as a protected variable (SSH_PRIVATE_KEY) in GitLab CI.
•	Added a deploy job in .gitlab-ci.yml that:
o	Sets up SSH agent and adds the private key.
o	Connects to the Azure VM via SSH.
o	Pulls the latest Docker image from DockerHub.
o	Stops and removes any existing container.
o	Runs the new container.

Key decision: Used StrictHostKeyChecking=no to bypass interactive confirmation in CI.
________________________________________
...
Command:
Key decision: Manually destroyed the instance instead of stopping the Docker container only.
...
What it shows:
A web browser displaying the live app at http://<vm-ip>:5000, with the name and date shown somewhere in the screenshot (e.g., terminal, tab title, or a sticky note).
Why it’s relevant to CI/CD security:
